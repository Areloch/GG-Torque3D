function AssetBrowser::buildPopupMenus(%this)
{
   ...
   if( !isObject( AddNewArtAssetPopup ) )
   {
      %this.AddNewArtAssetPopup = new PopupMenu( AddNewArtAssetPopup )
      {
         ...
         item[ 4 ] = "Create Shape Animation" TAB "" TAB "AssetBrowser.setupCreateNewAsset(\"AnimationAsset\", AssetBrowser.selectedModule);";//"AssetBrowser.createNewShapeAnimationAsset(\"NewShapeAnimation\", AssetBrowser.selectedModule);";
         item[ 5 ] = "-";
